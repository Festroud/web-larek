...
**Методы:**
- `renderHeaderBasketCounter(count: number): void` - Отображает количество товаров в корзине в заголовке. Количество передается через параметр `count`, который поступает из объекта корзины и обновляется при добавлении или удалении товаров.
...
**Методы:**
- `open(content: string = ''): void` - Открывает модальное окно с переданным содержимым.
...
#### SuccessModal
Назначение: Управляет отображением уведомления об успешном оформлении заказа.

**Методы:**

- `render(orderId: string, totalAmount: number): HTMLElement` - Создает и возвращает разметку уведомления об успешном заказе.
- `open(): void` - Открывает окно с уведомлением.
